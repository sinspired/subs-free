	...
	wailsApp.Event.On(updater.EventUpdateAvailable, func(e *application.CustomEvent) {
		application.InvokeAsync(func() {
			...
			if version != "" {
				...
				// 3. 发送系统通知提示用户
				sendOSNotification("Subs Free 更新", "发现新版本 v"+version+"，请及时更新")
			} else {
				checkUpdateItem.SetLabel("有新版本")
				sendOSNotification("Subs Free 更新", "发现新版本，请及时更新")
			}
		})
	})
...
func buildTrayMenu(
	...
	menu.Add("Subs Free 桌面端").SetBitmap(logo32).SetEnabled(false)
	...
	menu.Add("隐藏主界面").OnClick(func(_ *application.Context) {
		guiApp.hideActiveWindow()
		sendOSNotification("Subs Free", "已最小化到系统托盘\n单击图标可恢复窗口")
	})
	...
	menu.Add("返回登录窗口").OnClick(func(_ *application.Context) {
		guiApp.BackToLogin()
		sendOSNotification("Subs Free", "已返回登录窗口")
	})
	...
	triggerCheckMenu.OnClick(func(_ *application.Context) {
		...
		sendOSNotification("Subs Free", "已触发检测任务\n可在系统托盘、管理界面查看检测进度")
	...
	stopCheckMenu.OnClick(func(_ *application.Context) {
		if err := callBackendForceClose(); err != nil {
			...
			sendOSNotification("Subs Free", "已发送停止检测信号\n正在等待结果收集完成")
	...
	stopCheckAndExitMenu.OnClick(func(_ *application.Context) {
		if gracefulQuitPending.CompareAndSwap(false, true) {
			sendOSNotification("Subs Free", "正在等待检测完成后退出\n再次点击将立即强制退出")
			...
			gracefulQuitOnce.Do(func() {
				go func() {
					...
					sendOSNotification("Subs Free", "检测已完成，正在退出…")
					onQuit()
				}()
			})
		} else {
			slog.Warn("GUI：用户二次确认，立即退出")
			sendOSNotification("Subs Free", "正在强制退出…")
			onQuit()
		}
	})
	...
	autostartItem.OnClick(func(_ *application.Context) {
		...
		if err != nil {
			...
			sendOSNotification("Subs Free", "读取开机自启状态失败")
			return
		}
		...
		if next {
			...
			sendOSNotification("Subs Free", "已设置开机自启")
		} else {
			err = guiApp.autostart.Disable()
			sendOSNotification("Subs Free", "已取消开机自启")
		}
		...
		if err != nil {
			...
			sendOSNotification("Subs Free", "设置开机自启失败："+err.Error())
		}
	})
	...
	menu.Add("退出").OnClick(func(_ *application.Context) {
		slog.Info("GUI：强制退出")
		sendOSNotification("Subs Free", "正在关闭…")
		onQuit()
	})
...
func formatSysTrayTooltip(coreApp *app.App, guiApp *GuiApp) string {
	base := "Subs Free" + " - 端口 " + strings.TrimPrefix(config.GlobalConfig.ListenPort, ":")
